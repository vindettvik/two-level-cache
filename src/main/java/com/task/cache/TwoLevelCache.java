...
import com.task.cache.filesystem.FileSystemCache;
import com.task.cache.memory.MemoryCache;
import com.task.strategies.LFUStrategy;
import com.task.strategies.LRUStrategy;
import com.task.strategies.Strategy;
...
public class TwoLevelCache<K extends Serializable, V extends Serializable> implements Cache<K, V> {
    ...
    public TwoLevelCache(Integer memorySize, Integer fsSize, StrategyType strategyType) {
        this.firstLevel = new MemoryCache<>(memorySize);
        this.secondLevel = new FileSystemCache<>(fsSize);
